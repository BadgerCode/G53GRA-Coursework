...
std::vector<ObjectFace*> ObjectLoader::LoadObject(std::string name)
{
	...
	for (auto face : parsedJson.at("faces"))
	{
		...
		objectFace->TextureId = Scene::GetTexture(Materials::GetPath(face.at("materialName").get<std::string>()));
